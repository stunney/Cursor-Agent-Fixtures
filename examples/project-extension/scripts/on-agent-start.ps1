...
$line = "[on-agent-start] event=$event conversation=$conversationId subagent=$subagentId type=$subagentType dryRun=$dryRun"

Write-Output $line
Write-Output "workspace=$($payload.workspaceRoot)"

if ($dryRun) {
  Write-Output "Would append this start event to state/lifecycle.log"
  exit 0
}
...
Write-Output "Recorded start event in state/lifecycle.log"
exit 0
