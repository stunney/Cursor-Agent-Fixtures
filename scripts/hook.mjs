...
function runCommand(cwd, command, cached) {
  if (cached?.success) {
    return {
      success: true,
...
function getStageEntries(config, stage, runMode = "always") {
  return (config.stages[stage] ?? []).filter((entry) => entry.run === runMode);
}
function getOnDemandFlows(config) {
  return config.flows ?? [];
}
...
function recordStageResults(state, stage, results) {
...
function mergeStateData(state, data) {
...
async function runStage(project, config, state, stage, hookInput) {
...
async function executeFixture(project, config, state, entry, stage, hookInput) {
  if (entry.script) {
...
function runScriptFixture(project, config, entry, hookInput) {
...
function shouldAttemptRecovery(state, config, loopCount = 0) {
...
function recordRecoveryAttempt(state, ctx, message) {
  if (!state.recovery) {
...
function buildRecoveryFollowup(state, stage, loopCount = 0) {
...
function buildSetupFailureFollowup(state, stage) {
  const failure = getLastStageFailureMessage(state, stage);
  if (!failure) {
    return null;
  }
...
// src/stages.ts
function getStagesForHook(hookEvent, input) {
  switch (hookEvent) {
    case "sessionStart":
      return {
...
function isGateHook(hookEvent) {
  return hookEvent === "beforeSubmitPrompt";
}
function getConversationId(input) {
  const id = input.conversation_id ?? input.session_id ?? input.parent_conversation_id;
  return id ?? "unknown";
}
...
${fixtureResult.stdout}`
    ...
    if (hookEvent === "subagentStart" && stage === "agentSetup" && !result.success) {
      writeOutput({
        permission: "deny",
...
function buildNoOpResponse(hookEvent) {
...
function buildGateResponse(state) {
  if (requiredSetupFailed(state)) {
    const followup = buildSetupFailureFollowup(state, "multiAgentSetup");
    return {
      continue: false,
      user_message: "Multi-agent setup fixture failed. Fix .cursor/extensions/agent-fixtures/ config or state before continuing.",
      ...followup ? { agent_message: followup } : {}
    };
  }
  return { continue: true };
}
function buildHookResponse(hookEvent, input, project, config, state, allResults, lastFailedStage) {
  const summary = allResults.join("\n\n");
  const onDemand = getOnDemandFlows(config).map((flow) => `- ${flow.id} (${flow.process})`).join("\n");
